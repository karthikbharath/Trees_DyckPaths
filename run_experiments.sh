...
for arg in "$@"; do
  case ${arg} in
    ...
    --test-all)
    for method in Binomial Poisson Geometric Binary-0-1-2 Binary-0-2 ; do
      #for trials in 1000 5000 10000; do
      for trials in 1000; do
        for n in 11 101 1001; do
    ...
    --measure-lca)
      for ((i=1;i<=10;i++)); do
        for method in Binary-0-2; do
          numnodes=$((($RANDOM%10)+10))
          numsamples=`expr $numnodes*0.4 | bc -l | awk '{printf "%0.f", $1}'`
          filename=${method}-big-file.$i.txt
          cat <<EOF > options.$method.$i.txt;
--gen-method ${method}
--num-trials 100
--n $numnodes
--verbosity 0
...
--measure-mle false
--measure-lca true
--num-lca-samples $numsamples
--test-confidence false
--lambda 1
--k 2
--p 0.2
--dump-numbers false
--use-random-weights true
--num-threads 1
--dot-out foo.dot
EOF
        ./harness @options.${method}.$i.txt >> ${filename}
      done
    done
    ;;
...
echo "--measure-lca: running the LCA experiment"
    ;;
  esac
done
